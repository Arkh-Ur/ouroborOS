...
docs(user-guide): document ouroboros-upgrade workflow
```
...
1. **Root is read-only.** Any operation that writes to `/` in the installed system must use `ouroboros-upgrade`.
