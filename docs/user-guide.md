...
ouroborOS uses `pacman` for package management. Because the root filesystem is **read-only**, you must use the `our-pacman` wrapper instead of calling `pacman` directly. The wrapper:
...
# Install a package
sudo our-pacman -S neovim tmux htop

# Update the entire system
sudo our-pacman -Syu

# Remove a package
sudo our-pacman -Rns packagename
```

> **Do not use `sudo pacman -S/R/U` directly** — it will fail because the root
> filesystem is mounted read-only. `our-pacman` is the correct entry point
> for any write operation on the root filesystem.

> **Snapshot created automatically:** before every `our-pacman` invocation,
